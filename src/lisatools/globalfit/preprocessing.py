...
def find_file(folder: str, source_type: str, source_id: int) -> str:
    ...
    raise FileNotFoundError(f"No Mojito L1 file found for source type '{source_type}' and source ID '{source_id}' in folder '{folder}'.")
...
def _apply_filter(
        ...
        **kwargs
        ...
        if kwargs.get('do_plots', False):  
            decimate_factor = kwargs.get('decimate_factor', 1000)  # Decimate for plotting if too many points

            plt.figure(figsize=(10, 4))
            plt.plot(data_in[0, ::decimate_factor], label='Original X channel', c='gray', alpha=0.5)
    ...
    def __init__(self, 
        ...
        for source_type in source_types:
            if source_type not in ALLOWED_SOURCES:
                raise ValueError(f"Source type '{source_type}' is not allowed. Allowed types are: {ALLOWED_SOURCES}")
            ...
            for source_id in tqdm(ids, desc=f"Loading {source_type} sources", disable=not self.verbose):
                ...
                with self._open(file_path) as f:
                    ...
                    if xyz is None:
                        ...
                        assert (tdi_times == _tdi_times).all(), "Time arrays do not match between files."
                

        xyz = xyz.T  # Transpose to have shape (n_channels, n_times)
        assert xyz.shape[1] == tdi_times.shape[0], "Data time dimension does not match time array length."
...
class SignalProcessor:
    ...
    def __init__(self,
        ...
        self.T = times[-1] - times[0] # total duration. Takes into account that times[0] may not be zero.
        self.data = data
        self.fs = fs
        self.verbose = verbose
    ...
    def bandpass_filter(
        ...
        **kwargs: Additional keyword arguments for the eventual plot.
        ...
        """
        return _apply_filter(self.data, self.fs, low, high, 'bandpass', order, filter_type, zero_phase, axis=-1, do_plots=self.do_plots, **kwargs)
    ...
    def lowpass_filter(
        ...
        **kwargs: Additional keyword arguments for the eventual plot.
        ...
        filtered_data : np.ndarray
            Array of filtered channel data
        """
        return _apply_filter(self.data, self.fs, cutoff, None, 'lowpass', order, filter_type, zero_phase, axis=-1, do_plots=self.do_plots, **kwargs)
    ...
    def highpass_filter(
        ...
        **kwargs: Additional keyword arguments for the eventual plot.
        ...
        filtered_data : np.ndarray
            Array of filtered channel data
        """
        return _apply_filter(self.data, self.fs, cutoff, None, 'highpass', order, filter_type, zero_phase, axis=-1, do_plots=self.do_plots, **kwargs)
    ...
    def trim(
        ...
        **kwargs: Additional keyword arguments for the eventual plot.
        ...
        assert trimming_type in ['from_each_end', 'keep_from_start', 'discard_from_start'] #todo find proper name and implement 
        ...
        if trimming_type == 'from_each_end':
            ...
            if self.verbose:
                logger.info(f"Trimming {duration}s ({trim_samples} samples) from each end of the data.")
            ...
            if self.verbose:
                logger.info(f"Trimming {duration}s ({trim_samples} samples) from the start of the data.")
        ...
        if self.do_plots:
            decimate_factor = kwargs.get('decimate_factor', 1000)  # Decimate for plotting if too many points
            plt.figure(figsize=(10, 4))
            plt.plot(self.times[::decimate_factor], self.data[0, ::decimate_factor], label='Original X channel', c='gray', alpha=0.5)
            plt.plot(trimmed_times[::decimate_factor], trimmed_data[0, ::decimate_factor], label='Trimmed X channel', c='blue')
            plt.xlabel('Time [s]')
            plt.ylabel('Strain')
            plt.title('Signal Trimming')
            plt.legend()
            plt.savefig(os.path.join(kwargs.get('plot_folder', '.'), kwargs.get('filename', 'trim_signal.png')))
            plt.close()
...
class BaseProcessingStep(SignalProcessor):
    ...
    def __repr__(self):
        return (f"ProcessingStep with {self.N} samples, "
    ...
    def process(self,
        ...
        if do_detrend:
            ...
            assert self.data.shape[1] == self.times.shape[0], "Data and times length mismatch after detrending."
        ...
        if Tobs is not None:
            _, _ = self.trim(duration=Tobs, is_percent=False, trimming_type='keep_from_start', filename='science_data.png', **kwargs)
...
class SangriaProcessingStep(SangriaDataLoader, BaseProcessingStep):
    ...
    def __init__(self,
            ...
            SangriaDataLoader.__init__(self,
                            data_input_path=data_input_path,
                            remove_from_data=remove_from_data
            )
